    ...
    <div class="group-info-content">
      ...
      <div class="info-item">
        ...
        <div class="content" v-if="!showEditFaceUrl">
          <van-image round :src="groupProfile.avatar"/>
        </div>
        <van-field
            ref="editFaceUrl"
            v-else
      ...
      <van-button v-if="isOwner" size="mini" @click="showChangeGroupOwner = true">转让群组</van-button>
      <van-button size="mini" type="danger" @click="quitGroup">退出群组</van-button>
      <van-button size="mini" v-if="showDissmissGroup" type="danger" @click="dismissGroup">解散群组</van-button>
      <van-dialog
        show-cancel-button
        title="转让群"
        v-model="showChangeGroupOwner"
        @confirm="changeOwner">
        <van-field
          v-model="newOwnerUserID"
          placeholder="新群主的用户ID"
          size="mini"
        />
      </van-dialog>
    </div>
...
import { Field, Button, Image, Dialog } from 'vant';
...
export default {
  ...
  computed: {
    muteAllMembers() {
      return this.groupProfile.muteAllMembers;
    },
    editable() {
      return (
        this.groupProfile.type === this.TIM.TYPES.GRP_WORK
        || [this.TIM.TYPES.GRP_MBR_ROLE_OWNER, this.TIM.TYPES.GRP_MBR_ROLE_ADMIN]
